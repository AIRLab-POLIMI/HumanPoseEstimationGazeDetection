...
NNAAMMEE
       oommxxppllaayyeerr - Raspberry Pi command line OMX player

       OMXPlayer  is a commandline OMX player for the Raspberry Pi. It was de‐
       veloped as a testbed for the XBMC Raspberry PI  implementation  and  is
...
KKEEYY CCOONNFFIIGG SSYYNNTTAAXX
       A key config file is a series of rules of the form [action]:[key]. Mul‐
       tiple keys can be bound to the same action, and comments are  supported
       by  adding  a  #  in  front  of  the line.  The list of valid [action]s
       roughly corresponds to the list of default key bindings above and are:
       ...
       Valid [key]s include all alpha-numeric characters and most symbols,  as
       well as:
